...
$subjects = [];
$sql = "SELECT id, subject FROM tb_answers WHERE schoolid = 101010 AND studname = 'Answer_key' AND subject LIKE '%" . $_SESSION["subjectarea"] . "%' ORDER BY subject ASC";
...
<body>
    <div class="container mt-5">
        <h1 class="text-center mb-4" style="color: #000000;">Search Answers</h1>

        <?php
        $results = [];
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search'])) {
            $selectedSubject = $_POST['file'];
            $sql = "SELECT * FROM tb_answers WHERE  id = ?";
                        ...
                        <?php foreach ($results as $row): ?>
                            ...
                            <?php for ($i = 1; $i <= 50; $i++): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row["a$i"]); ?></td>
                                </tr>
                            <?php endfor; ?>
                        <?php endforeach; ?>
                    </tbody>
